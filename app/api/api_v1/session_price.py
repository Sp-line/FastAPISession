...
@router.get("/")
async def get_session_prices(
        service: FromDishka[SessionPriceService],
...
@router.post("/")
async def create_session_price(
        data: SessionPriceCreateReq,
        service: FromDishka[SessionPriceService]
) -> SessionPriceRead:
...
@router.post("/bulk")
async def bulk_create_session_prices(
        data: list[SessionPriceCreateReq],
        service: FromDishka[SessionPriceService]
) -> list[SessionPriceRead]:
...
async def delete_session_price(
        session_price_id: int,
        service: FromDishka[SessionPriceService]
) -> None:
